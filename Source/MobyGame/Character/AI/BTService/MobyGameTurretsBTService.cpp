...
#include "MobyGameTurretsBTService.h"
...
void UMobyGameTurretsBTService::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
			...
			if (AMobyGameCharacter* OwnerCharacter = Cast<AMobyGameCharacter>(OwnerController->GetPawn()))
			{
				if (UBlackboardComponent* MyBlackBoard = OwnerComp.GetBlackboardComponent())
				{
					...
					if (InTarget)
					{
						...
						if (Distance > RangeAttack /*2000.f*/ || InTarget->IsDie())
						{
...
void UMobyGameTurretsBTService::InitializeFromAsset(UBehaviorTree& Asset)
{
